...
async function main() {
  ...
  if (!filePath) { console.error('Usage: node scripts/import_through_9_2_eob.mjs "through-9-2-eob.csv"'); process.exit(1); }
  ...
  try {
    ...
    const ins = await client.query(
      `INSERT INTO uploads (filename, sha256_digest, source, week_label) VALUES ($1,$2,'timecard_csv','through-9-2-eob') RETURNING id`, 
      [filename, digest]
    );
    const uploadId = ins.rows[0].id;
    console.log(`Created upload record with ID: ${uploadId}`);
    
    let rowsIngested = 0, segs = 0, skipped = 0, lunchBreaks = 0;
    ...
    for (let li = 1; li < records.length; li++) {
      ...
      try {
        ...
        await client.query(
          ...
          [driverId, uploadId, startUtc.toISO(), endUtc.toISO(), JSON.stringify({ src: 'through-9-2-eob', line: li })]
        );
    ...
    console.log(`Lunch breaks detected: ${lunchBreaks}`);
